    ...
    <p-tabView>
      <p-tabPanel header="Se connecter">
        <form
          ...
          @if (signInError()) {
            <span class="text-sm text-red-500"
              >Adresse e-mail ou mot de passe incorrect.</span
            >
          }
          <div class="flex gap-4">
            <app-primary-button type="submit">Se connecter</app-primary-button>
            <app-secondary-button (click)="onClose()"
              >Fermer</app-secondary-button
            >
          </div>
        </form>
      </p-tabPanel>
      <p-tabPanel header="S'inscrire">
        <form
          ...
          @if (signUpError()) {
            <span class="text-sm text-red-500"
              >L'inscription a échoué. Veuillez réessayer.</span
            >
          }
          <div class="flex gap-4">
